...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 26 12:35:23 2022
...
basket_img = pygame.transform.scale(basket_img, (150, 20))
...
class Basket(object):
    def __init__(self, x, y):
        ...
        self.y = y
        self.vel = 10
        self.hitbox = (self.x - 20, self.y - 20, 60, 80)
    ...
    def draw(self, window):
        if self.f_type == 0:
            ...
            self.vel = 10
             ...
             fruit = pygame.image.load("orange.png")
             self.vel = 20
...
def main():
    ...
    while play:
        ...
        if fruit_add_counter == add_fruit_rate:
            ...
            f_starty = 0
            f_type = random.randint(0, 1) # change to random later
        ...
        for item in fruits[:]:
            if (item.hitbox[0] >= basket.hitbox[0] - 20) and (item.hitbox[0] <= basket.hitbox[0] + 70):
                if basket.hitbox[1] - 120 <= item.hitbox[1] <= basket.hitbox[1] - 40:
                    ...
                    if item.f_type == 1:
                        TIME_FOR_POWERUP = pygame.time.get_ticks()
                        basket.vel += 10
                        #if TIME_FOR_POWERUP 
                        #    basket.vel -= 3
                        score += 0
                    print("Score:", score)
                    print("Basket Speed: ", basket.vel)
        for item in bombs:
            item.draw(window)
            item.y += item.vel
        for item in bombs[:]:
        ...
        basket.draw(window)
        pygame.draw.rect(window,(160, 160, 160), basket.hitbox)
        pygame.display.update()
        clock.tick(60)
    pygame.quit()
